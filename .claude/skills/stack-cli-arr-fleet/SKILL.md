...
description: Exact fish CLI command reference for Radarr/Sonarr operations against this stack's Control Panel (queue, backlog, missing/cutoff-unmet items, manual import, RSS sync, unstick). Use whenever the user asks to check, search, import, or unstick something in Radarr or Sonarr from the terminal, or asks what stack-arr-* commands exist. Trigger phrases: "search missing on radarr", "what's stuck in sonarr's queue", "import this file", "check the arr backlog", "cutoff unmet", "recently added to radarr".
---
...
<resources>
**Local:**
- `~/.config/fish/functions/stack-arr*.fish`, `stack-cutoff-unmet.fish`, `stack-import-lists.fish`, `stack-recently-added.fish`, `stack-command-queue-summary.fish`, `stack-queue-status.fish`, `stack-backlog-status.fish` - the actual fish source these commands wrap
