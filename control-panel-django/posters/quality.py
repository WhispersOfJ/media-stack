"""Poster quality heuristics for the bulk scan job, ported verbatim from
the FastAPI-era control-panel/services/posters/quality.py for the
Django/DRF rewrite. Given an item's Plex metadata, its media type, and the
raw bytes of its currently-applied poster, return a list of flag strings.
Pure functions, no I/O of their own (the caller already fetched the
bytes/metadata) - same "pure lookup" isolation as candidates.py.

No transform needed vs. the FastAPI-era source - this module never called
core.responses.fail(), so nothing to swap for ServiceError. Only the
`from services.posters.candidates import ...` path changed to
`from posters.candidates import ...`.
"""
import io

from PIL import Image

from posters.candidates import TMDB_KEY, tmdb_get, tmdb_id_for_item
