...
ported near-verbatim from the FastAPI-era
control-panel/services/posters/candidates.py for the Django/DRF rewrite.
...
core.api_base.ServiceError, matching every other ported app. Every other
constant/function name and signature is byte-identical.
"""
import os
import threading
import time
...
def tmdb_get(path: str, **params) -> dict:
    if not TMDB_KEY:
        raise ServiceError("TMDb isn't configured (TMDB_KEY not set)", status=503)
