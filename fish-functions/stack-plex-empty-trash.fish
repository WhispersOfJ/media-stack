...
function stack-plex-empty-trash --description 'Empty trash on one Plex library, or all of them'
    ...
    if test (count $argv) -eq 0
        curl -sS -H "X-Api-Key: $service_key" -X POST "http://$host_ip:8420/api/v2/plex/empty-trash" | python3 -c "
import json, sys
...
"
    ...
    curl -sS -H "X-Api-Key: $service_key" -X POST "http://$host_ip:8420/api/v2/plex/empty-trash?library=$encoded" | python3 -c "
import json, sys
d = json.load(sys.stdin)
if isinstance(d, dict) and isinstance(d.get('detail'), dict):
    d = d['detail']
