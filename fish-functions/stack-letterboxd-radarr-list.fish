...
function stack-letterboxd-radarr-list --description 'Add every film in a Letterboxd list to Radarr'
    ...
    if test (count $argv) -ne 1
        echo "Usage: stack-letterboxd-radarr-list <letterboxd-list-url> [--no-search] [--no-monitor] [--limit N] [--dry-run]" >&2
        return 1
    end
    set -l url $argv[1]
    set -l search true
...
import json, sys
url, search, monitored, limit, dry_run = sys.argv[1:6]
payload = {'url': url, 'search': search == 'true', 'monitored': monitored == 'true', 'dry_run': dry_run == 'true'}
if int(limit) > 0:
    payload['limit'] = int(limit)
print(json.dumps(payload))
" "$url" "$search" "$monitored" "$limit" "$dry_run")
