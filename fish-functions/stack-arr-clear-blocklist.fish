# Usage: stack-arr-clear-blocklist <radarr|sonarr|radarr_anime|sonarr_anime> [-y|--yes]
...
function stack-arr-clear-blocklist --description 'Clear every blocklisted release in Radarr/Sonarr'
    if test (count $argv) -lt 1; or not __stack_arr_app $argv[1] >/dev/null
        echo "Usage: stack-arr-clear-blocklist <radarr|sonarr|radarr_anime|sonarr_anime> [-y|--yes]" >&2
        return 1
    end
