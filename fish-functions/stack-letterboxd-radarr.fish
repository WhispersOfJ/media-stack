# Usage: stack-letterboxd-radarr <letterboxd-film-url> [--no-search] [--no-monitor] [--dry-run] [--anime]
...
function stack-letterboxd-radarr --description 'Add a Letterboxd film to Radarr by URL'
    argparse 'no-search' 'no-monitor' 'dry-run' 'anime' -- $argv
    or return 1
    if test (count $argv) -ne 1
        echo "Usage: stack-letterboxd-radarr <letterboxd-film-url> [--no-search] [--no-monitor] [--dry-run] [--anime]" >&2
        return 1
    end
    set -l url $argv[1]
    set -l search true
