...
function stack-letterboxd-radarr --description 'Add a Letterboxd film to Radarr by URL'
    ...
    set -l body (python3 -c "import json, sys; print(json.dumps({'url': sys.argv[1], 'search': sys.argv[2] == 'true', 'monitored': sys.argv[3] == 'true', 'dry_run': sys.argv[4] == 'true'}))" "$url" "$search" "$monitored" "$dry_run")
