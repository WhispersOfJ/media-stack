...
import json, sys
url, search, monitored, dry_run, limit = sys.argv[1:6]
payload = {'list_url': url, 'search': search == 'true', 'monitored': monitored == 'true', 'dry_run': dry_run == 'true'}
if int(limit) > 0:
    payload['limit'] = int(limit)
print(json.dumps(payload))
" "$url" "$search" "$monitored" "$dry_run" "$limit")
