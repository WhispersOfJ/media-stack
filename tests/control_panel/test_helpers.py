...
def test_bucket_plex_activity_downloading(cp_app):
    a = {"uuid": "u1", "title": "Scan", "progress": 60}
    bucket, item = cp_app._bucket_plex_activity(a, {"u1": 30})
    assert bucket == "downloading"
    assert item["progress"] == "60%"


def test_bucket_plex_activity_stalled(cp_app):
    a = {"uuid": "u1", "title": "Scan", "progress": 30}
    bucket, item = cp_app._bucket_plex_activity(a, {"u1": 30})
    assert bucket == "stalled"


def test_bucket_plex_activity_with_subtitle(cp_app):
    a = {"uuid": "u1", "title": "Scan", "subtitle": "Movies", "progress": 10}
    bucket, item = cp_app._bucket_plex_activity(a, {})
    assert item["title"] == "Scan: Movies"
