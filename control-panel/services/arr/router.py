...
def mdblist_import_list(payload: MDBListImportRequest, _=Depends(current_user_or_service)):
    ...
    match = MDBLIST_URL_RE.match(payload.list_url.strip().rstrip("/") + "/")
    if not match:
    ...
    while True:
        ...
        try:
            r = httpx.get(f"https://mdblist.com/api/lists/{username}/{listname}/items", params=params, timeout=20)
            r.raise_for_status()
        except httpx.HTTPError as e:
        ...
        if payload.limit and (len(movies) + len(shows)) >= payload.limit:
            break
    ...
    if movies:
        ...
        for m in movies:
            ...
            if not tmdb_id:
                ...
                continue
            r = radarr_add_movie(radarr_cfg, tmdb_id, payload.monitored, payload.search, radarr_root_folder_path,
                                  radarr_quality_profile_id, existing_tmdb_ids, dry_run=payload.dry_run)
    ...
    if shows:
        ...
        for s in shows:
            ...
            if not tvdb_id:
                ...
                continue
            r = sonarr_add_series(sonarr_cfg, tvdb_id, payload.monitored, payload.search, sonarr_root_folder_path,
                                   sonarr_quality_profile_id, existing_tvdb_ids, dry_run=payload.dry_run)
            (added if r["status"] == "added" else already if r["status"] == "already" else failed).append(
